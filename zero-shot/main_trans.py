...
def train(args, model, train_loader, optimizer, criterion, criterion_event, epoch):
	...
	for batch_idx, sample in enumerate(train_loader):
		...
		if args.dataset_name in ['AVE', 'vggsound', 'LLP']:
			event_scores, logits_audio_image, logits_image_audio = model(wave.to('cuda'), image.to('cuda'))
		...
		if epoch <= 10:
			...
			w2 = loss_audio_image / (epoch + loss + loss_audio_image + loss_image_audio) + 1 / epoch
			w3 = loss_image_audio / (epoch + loss + loss_audio_image + loss_image_audio) + 1 / epoch
	...
	for batch_idx, sample in enumerate(val_loader):
		...
		if args.dataset_name in ['AVE', 'vggsound', 'LLP']:
			...
			if not args.classification:
				assert args.dataset_name != "LLP", "LLP dataset can only do classification task!"
				val = (event_scores.argmax(dim=-1) == rearrange(labels, 'b t class -> (b t) class').argmax(dim=-1)).sum()
			else:
...
def main():
	...
	if args.mode == 'train':
		...
		val_loader = DataLoader(val_dataset, batch_size=4, num_workers=args.num_workers, shuffle=False, pin_memory=True)
