...
class AVE_dataset(Dataset):
	...
	def __init__(self, opt, mode='train'):
		...
		if self.dataset_name == "vggsound":
			all_df = pd.read_csv(os.path.join(self.root_path, "data/vggsound/vggsound-avel40k_labels.csv"))
			self.split_df = all_df[all_df['split'] == mode]

			# 输出train、test、valid的占比
			print(f'{len(self.split_df)}/{len(all_df)} videos are used for {mode}')
			self.all_categories, self.id_to_idx = generate_category_list(self.dataset_name)
			...
			if mode == 'train':
				...
				with h5py.File(os.path.join(self.root_path, 'data/AVE/test_order.h5'), 'r') as hf:
					order = hf['order'][:]

			self.raw_gt = pd.read_csv(os.path.join(self.root_path, "data/AVE/Annotations.txt"), sep="&")
			category_dict = dict()
			for category in self.all_categories:
				category_dict[category] = []
		...
		if self.dataset_name == "vggsound":
			...
			for vis_idx in range(10):
				...
				tmp_img = torchvision.io.read_image(
					self.opt.video_folder + '/' + file_name + '/' + str("{:08d}".format(tmp_idx)) + '.jpg') / 255
			...
			wave = np.load(os.path.join(self.root_path, 'AVVP/LLP_dataset/wave/{}.npy'.format(file_name)), allow_pickle=True)
			wave = torch.from_numpy(wave)
			wave = wave.view(10, 32000)
			while wave.size(-1) < 32000 * 10:
				wave = torch.cat((wave, wave), dim=-1)     
