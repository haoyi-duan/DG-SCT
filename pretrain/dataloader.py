...
class AVE_dataset(Dataset):
	...
	def __getitem__(self, idx):
		if self.dataset_name == "vggsound":
			...
			wave = np.load(os.path.join(self.root_path, 'data/vggsound/wave/{}.npy'.format(file_name)), allow_pickle=True)
			...
			for vis_idx in range(10):
				tmp_idx = sample_indx[vis_idx]
				tmp_img = torchvision.io.read_image(os.path.join(self.root_path, self.video_folder)+'/'+file_name+'/'+ str("{:08d}".format(tmp_idx))+ '.jpg')/255
				tmp_img = self.my_normalize(tmp_img)
				total_img.append(tmp_img)
			total_img = torch.stack(total_img)
			### <---
